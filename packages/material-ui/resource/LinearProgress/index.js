...
function LinearProgress(props) {
  ...
  if (variant === 'determinate' || variant === 'buffer') {
    if (value !== undefined) {
      ...
      warning(
        false,
        'Material-UI: you need to provide a value property ' + 'when using the determinate or buffer variant of LinearProgress .'
      );
    }
  }
  if (variant === 'buffer') {
  ...
  return (
    <div className={className} role="progressbar" {...rootProps} {...other}>
      ...
      {variant === 'determinate' ? null : <div className={bar2ClassName} style={inlineStyles.bar2} />}
    </div>
  );
}
