    ...
    <MergeListContext dense={denseProp} alignItems={alignItems}>
      {({ dense }) => {
        ...
        if (hasSecondaryAction) {
          ...
          return (
            <ContainerComponent className={classNames(classes.container, ContainerClassName)} {...ContainerProps}>
    ...
    if (secondaryActionIndex !== -1 && secondaryActionIndex !== children.length - 1) {
      return new Error(
        'Material-UI: you used an element after ListItemSecondaryAction. ' +
          ...
          `you must pass it has the last children to ListItem.${process.env.NODE_ENV === 'test' ? Date.now() : ''}`
      );
    }
